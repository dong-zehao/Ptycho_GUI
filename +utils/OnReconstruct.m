function OnReconstruct(app)
    ...
    for i_run = 1:size(app.UITable.Data,1)
        ...
        switch app.RotateSwitch.Value
            case 'Rot. CBED'
                rot_ang = 0;
                utils.LogMessage(app, 'No scan rotation during reconstruction');
            case 'Rot. Scan'
                rot_ang = app.ScanRotSpinner.Value;
                utils.LogMessage(app, sprintf('Scan rotation %.1f during reconstruction', rot_ang));
            otherwise
