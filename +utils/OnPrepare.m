function OnPrepare(app)
    ...
    for i_run = 1:size(app.UITable_2.Data,1)
        ...
        switch app.RotateSwitch.Value
            case 'Rot. CBED'
                ...
                utils.LogMessage(app, sprintf('Finished CBED rotating by %.1f degrees', tot_rotation));
            case 'Rot. Scan'
    ...
    % Step 2: prepare data
        ...
        % recenter
        if icenter==1
            utils.LogMessage(app, 'Recentering CBED ......');
        ...
        if ndpy < Np_p(1) % pad zeros
            utils.LogMessage(app, sprintf('Padding zero into CBED, size %i x %i', Np_p(1), Np_p(2)));
            cbed=padarray(cbed,[(Np_p(1)-ndpy)/2,(Np_p(2)-ndpx)/2,0,0],0,'both');
        else
            utils.LogMessage(app, sprintf('Cropping CBED, size %i x %i', Np_p(1), Np_p(2)));
