...
elseif p.prepare.legacy
   ...
   low_photon_count_dp = squeeze(sum(sum(p.fmag))) == 0 ;
    if any(low_photon_count_dp)
        ...
        p.numpts=p.numpts-length(p.idx_all0);
        p.numpos=p.numpos-length(p.idx_all0);
        p.scanindexrange(end)=p.scanindexrange(end)-length(p.idx_all0);
        p.fmag(:,:,low_photon_count_dp)=[];        
        p.positions(low_photon_count_dp,:)=[];
        p.positions_real(low_photon_count_dp,:)=[];
        p.positions_orig(low_photon_count_dp,:)=[]; 
        p.scanidxs{1}=p.scanidxs{1}(1:end-length(p.idx_all0));
