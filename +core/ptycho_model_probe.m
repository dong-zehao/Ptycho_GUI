...
% if p.detector.upsampling >0 % commented out by ZC
...
%         w = (r2 < (r1_pix)^2);
    ...
    while ~fileokflag
        try
            ...
            probe = S.probe;
            probe = probe(:,:,:,:); %%added by YJ. Force to ignore the 4-th dimension (used for storing OPR modes) recover by dzh
            %disp(size(probe))
    ...
    elseif ndims(probe) == 4 % modified by dzh: need to swap dimension 3 and 4
        sz_pr = size(probe);
        probe = permute(probe, [1 2 4 3]);
    end
