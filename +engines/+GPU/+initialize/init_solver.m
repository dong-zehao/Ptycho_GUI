    ...
    if par.precession_mode
        for ilayer = 1:par.Nlayers
            for iscan = 1:par.Nscans
                ...
                self.modes{ilayer}.probe_positions(ind_scan,:) = self.modes{ilayer}.probe_positions(ind_scan,:) + self.modes{ilayer}.interlayer_shift_position{iscan}./self.pixel_size;
                self.modes{ilayer}.probe_positions_0(ind_scan,:) = self.modes{ilayer}.probe_positions_0(ind_scan,:) + self.modes{ilayer}.interlayer_shift_position{iscan}./self.pixel_size;
            end
        end
    end
