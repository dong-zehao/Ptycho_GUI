...
%             if isempty(self.modes{1}.ASM_factor)  % is not nearfield % comment out by ZC
...
%             end
            ...
            % apply blur correction, change to Gaussian blurring by ZC
%             if self.diff_pattern_blur > 0 % comment out by ZC
                % generate blurring kernel 
                blur_kernel = fspecial('gaussian',round(self.diff_pattern_blur)*10+1,self.diff_pattern_blur);
				aPsi2 = fftshift(fftshift(aPsi2,1),2); % correct low intensity near the edge pixels.
                aPsi2 = convn(aPsi2, blur_kernel, 'same');  
				aPsi2 = ifftshift(ifftshift(aPsi2,1),2);
%             end
