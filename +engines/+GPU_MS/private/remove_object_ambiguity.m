...
function self = remove_object_ambiguity(self, cache, par)         
   ...
   for kk = 1:size(self.object,1)
        % debug for scan rotation: dzh
        illum_temp = cache.illum_sum_0{min(kk,end)};
        illum_rot = imrotate(illum_temp, rot,"bilinear", 'crop' );

        W = illum_rot(cache.object_ROI{:});
        size_temp = size(W);
        idx_x = fix(size_temp(1)/4):fix(size_temp(1)*3/4);
        idx_y = fix(size_temp(2)/4):fix(size_temp(2)*3/4);
        W = W(idx_x, idx_y);
        W = W ./ math.norm2(W); 
        for layer = 1:par.Nlayers
            temp_obj = self.object{kk,layer};
            rot_obj = imrotate(temp_obj, rot,"bilinear", 'crop' );
            rot_obj = rot_obj(cache.object_ROI{:});
            rot_obj = rot_obj(idx_x, idx_y);
            object_norm(kk, layer) = sqrt(mean2(abs(rot_obj).^2.* W)); 
        end
    end
    if par.share_probe
