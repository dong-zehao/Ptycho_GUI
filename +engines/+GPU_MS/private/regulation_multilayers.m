...
%         %% amplitude constrain to phase *0.1 by Zhen Chen
% %         reg_amp=0.5;
%         reg_amp=par.reg_amp;
%         if reg_amp ~= 0
%             for ii = 1:par.Nlayers   
%                 temp= math.unwrap2D_fft2(obj_upd(:,:,ii),[],0,Wphase,-1);
%                 obj_upd(:,:,ii) = exp(1i*angle(obj_upd(:,:,ii))).*((1-temp*0.1)*reg_amp+abs(obj_upd(:,:,ii))*(1-reg_amp)); 
%             end
%         end
        %%
