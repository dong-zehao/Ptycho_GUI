...
function cache = precalculate_ROI(self, cache, probe)
    ...
    probe_position = self.probe_positions_0 * cache.affine_matrix_init';

    object_ROI = {round(self.Np_o(1)/2+(min(probe_position(:,2)):max(probe_position(:,2)))), ...
        round(self.Np_o(2)/2+(min(probe_position(:,1)):max(probe_position(:,1))))};
