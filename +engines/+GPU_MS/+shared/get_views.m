...
function obj_proj = get_views(object, obj_proj,layer_ids,object_id, indices, cache, scan_ids, skip_ind)
    % modified by dzh: all oROI modified to different layers
   ...
   skip_ind = [skip_ind, cache.skip_ind];
    ...
    if nargin > 6   
        ...
        if (length(unq_scans)> 1 || length(object) > 1 ) && iscell(object)
           if  ~isempty(use_gpu) && use_gpu && isa(object{object_id(1)}, 'gpuArray') && isa(obj_proj, 'gpuArray') 
               ...
               obj_proj = get_views_gpu(object(:,layer_ids),obj_proj,cache.oROI_s{min(end,layer_ids)}, indices, ind_ok);
           else          
    ...
    if isempty(obj_proj)
        Np_p = size(cache.ASM_difference); % modified by dzh to avoid using oROI
    ...
    if isa(object, 'gpuArray')
        ...
        obj_proj = get_views_gpu(object,obj_proj,cache.oROI_s{min(end,layer_ids)}, indices, ind_ok);
    else
        %% USE CPU 
        positions = int32([cache.oROI_s{min(end,layer_ids)}{1}(indices,1), cache.oROI_s{min(end,layer_ids)}{2}(indices,1)]);
