...
function object = set_views(object, object_update, layer,object_id, indices, cache, scan_ids, skip_ind)
    ...
    if length(indices) > 65536
        error('indices beyond 65536!')
    end
        ...
        if length(unq_scans)> 1
           if ~isempty(use_gpu) && use_gpu && isa(object{1}, 'gpuArray') && isa(object_update, 'gpuArray') 
               ...
               % feed data directly to the GPU mex without splitting 
                object(:,layer) = set_views_gpu(object(:,layer),object_update,cache.oROI_s{layer},indices, ind_ok); 
           else
    ...
    if isa(object, 'gpuArray')
        ...
        object = set_views_gpu(object,object_update,cache.oROI_s{min(layer,end)},indices,ind_ok); 
    else
        %% USE CPU
        positions = int32([cache.oROI_s{min(end,layer)}{1}(indices,1), cache.oROI_s{min(end,layer)}{2}(indices,1)]);
