    ...
    for i = 1:par.Nlayers % modified by ZC
        ...
        if is_used(par, 'fly_scan') && (~isfield(modes{i}, 'probe_positions') || isempty(modes{i}.probe_positions) )
           ...
           %% get positions for normal tomo 
            ...
            % modified by DZH: shift each layer according to its tilt angle in TCMEP
            if par.precession_mode
                verbose(2,'Providing interlayer shift used in TCMEP reconstructions')
                for iscan = 1:par.Nscans
                    ind_scan = self.reconstruct_ind{iscan};
                    % calculate shifted position for each layer
                    layer_shift_TCMEP = (sum(self.z_distance(1:i-1))-par.precession_vacuum_thickness) * sind(par.precession_angles{iscan});
                    modes{i}.probe_positions(ind_scan,:) = modes{i}.probe_positions(ind_scan,:) + layer_shift_TCMEP./self.pixel_size + par.refine_shift{iscan};
                    modes{i}.probe_positions_0(ind_scan,:) = modes{i}.probe_positions_0(ind_scan,:) + layer_shift_TCMEP./self.pixel_size + par.refine_shift{iscan};
                end
            end 
        end
...
%             ASM = fftshift(ASM);
    ...
    %% updated illumination; modified by dzh to include TCMEP
    if par.precession_mode
        layerid = ceil(par.precession_vacuum_thickness/self.z_distance(1));
    else
        layerid = 1;
    end
    [cache.oROI_s{layerid}] = find_reconstruction_ROI( modes{layerid}.probe_positions,self.Np_o, self.Np_p);
    illum_sum_0{1} = Gzeros(self.Np_o);
    for ll = 1:par.Nscans
        aprobe2 = abs(self.probe{1}(:,:,ll)).^2; 
        ind = [self.reconstruct_ind{ll}];
        ...
        for ipart = 1:max(ind_parts)
            ...
            illum_sum_0{1} = set_views(illum_sum_0{1}, Garray(aprobe2), layerid,1, ind_temp, cache);
        end
        % avoid oscilations by adding momentum term 
    end
    illum_sum_0{1} = illum_sum_0{1}/par.Nscans;
    ...
    switch par.init_layer_preprocess
        case 'avg' % only use the averaged layer
            ...
            for ll = 1:1 %par.Nscans % modified by dzh to include TCMEP
        ...
        case 'interp2' % add interpolation layers, ZC, not specify layers for interp but use Nlayers
            verbose(0,'Interpolate %d initial layers to %d layers uniformly', size(self.object,2), par.Nlayers)
            for ll = 1:par.Nscans
                obj_temp = cat(3,self.object{ll,:});
                [N_obj_y,N_obj_x,N_obj_z] = size(obj_temp); 
                [X,Y,Z] = meshgrid(linspace(1,N_obj_x,N_obj_x),linspace(1,N_obj_y,N_obj_y),linspace(1,N_obj_z,N_obj_z));
                [Xq,Yq,Zq] = meshgrid(linspace(1,N_obj_x,N_obj_x),linspace(1,N_obj_y,N_obj_y),linspace(1,N_obj_z,par.Nlayers));
                obj_temp = interp3(X,Y,Z,obj_temp,Xq,Yq,Zq,'spline');
                R_r=N_obj_z/par.Nlayers; % rescale phase change due to more layers
                for ii=1:par.Nlayers 
                    self.object{ll,ii}=abs(obj_temp(:,:,ii)).*exp(1i*phase_unwrap(angle(obj_temp(:,:,ii)))*R_r); 
    %                 obj{ll}{ii}=abs(obj_temp(:,:,ii)).*exp(1i*phase_unwrap(angle(obj_temp(:,:,ii)))*R_r);                        
                end
            end
    ...
    if size(self.object,2) > par.Nlayers
        warning('Initial object has more layers than Nlayers, use the first Nlayers') % by ZC
%         for ll = 1:par.Nscans
		self.object(:,par.Nlayers+1:size(self.object,2))=[];
...
%                     obj_pre = ones(self.Np_o, 'single');
    ...
    % modified by dzh to include TCMEP multiple scans
    illum_sum_0{1} = Ggather(illum_sum_0{1}); 
    cache.MAX_ILLUM(1) = max(illum_sum_0{1}(:));
    cache.illum_sum_0{1} = illum_sum_0{1}; 

    %% precalculate illumination ROIs
    [~,~,rot,~] = decompose_affine_matrix(par.affine_matrix_init);
    cache.init_rot = rot;
    cache.affine_matrix_init = par.affine_matrix_init;
