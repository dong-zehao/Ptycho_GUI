...
function [self, param, p] = load_from_p(param, p)
    ...
    % modified by DZH: precession mode
    if ~isfield(p, 'precession_mode')
        param.precession_mode = false;
    else
        param.precession_mode = p.precession_mode ;
        param.precession_angles = p.precession_angles;
        param.precession_vacuum_thickness = p.precession_vacuum_thickness;
    end
    if ~isfield(p, 'iter_correct_angle')
        param.iter_correct_angle = -1;
    else
        param.iter_correct_angle = p.iter_correct_angle ;
    end
    
    if ~isfield(p, 'correct_angle')
        param.correct_angle = false;
    else
        param.correct_angle = p.correct_angle;
    end
    if ~isfield(p, 'layer4pos')
        param.layer4pos = ceil(length(p.object)/2);
    else
        param.layer4pos = p.layer4pos;
    end
    if ~isfield(p, 'refine_shift')
        param.refine_shift = {[0 0]};
        param.refine_shift = repmat(param.refine_shift,[1 length(self.reconstruct_ind)]);
    else
        param.refine_shift = p.refine_shift ;
    end
    ...
    if ~check_option(p,'asize_presolve') 
        ...
        % correct bugs when upsampling not 0 by ZC
        if param.upsampling_data_factor > 0
            p.asize_presolve = p.asize_presolve .* 2^param.upsampling_data_factor;
        end
    ...
    self.noise  = [];
    self.diffraction = (single(bsxfun(@times,p.fmag, p.fmask)) / single(p.renorm) ).^2; % support 2D fmask by ZC
    self.mask = (~p.fmask);
    ...
    if any(low_photon_count_dp)
        error('%0.4f%% diffraction patterns has average photon count < %f', sum(low_photon_count_dp)/size(self.diffraction,3)*100, avg_photon_threshold)
    end
    ...
    if check_option(p,'diff_pattern_blur')
        self.diff_pattern_blur = p.diff_pattern_blur;  % add by ZC for detector smoothing. 1-2 may be a good approx.
    else
