...
function plot_geom_corrections(self, mode, object, iter, par, cache)
	...
	if iter >= par.probe_position_search
        % plot position correction results: by dzh
        layer = round(mean(par.layer4pos));
        plotting.smart_figure(106666)
        clf()
        for index = 1:par.Nscans
            ind = self.reconstruct_ind{index};
            set(gcf,'Outerposition',[100 100 800 800])    %[left, bottom, width, height]
            scatter(self.modes{layer}.probe_positions(ind,1), self.modes{layer}.probe_positions(ind,2) ,...
                60, '.', 'DisplayName', ['Dataset #' num2str(index)]); 
            hold on;
        end
        axis equal
        legend(Location="best")
        hold off 
        title('Refined probe positions')
        set(gca,'FontSize',25);
        set(gca,'box','on');
