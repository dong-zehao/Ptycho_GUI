...
function [ self,m_probe_update, probe_update, cache]  = ...
    ...
    if (probe_id == 1 && par.apply_subpix_shift && isinf(self.z_distance(end))) || is_used(par,'fly_scan')
        probe_update  = apply_subpx_shift(probe_update , -self.modes{1}.sub_px_shift(g_ind,:) );
    end
    if probe_id == 1
        probe_update = apply_subpx_shift_fft(probe_update, -self.modes{1}.probe_fourier_shift(g_ind,:));
    end
